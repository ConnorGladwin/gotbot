...
func GetPassword(db *sql.DB, id string) map[string]string {
  ...
  usernameQuery := fmt.Sprintf("select password from users where username = '%v'", id)
  emailQuery := fmt.Sprintf("select password from users where email = '%v'", id)

  usernameErr := db.QueryRow(usernameQuery).Scan(&userPassword)
